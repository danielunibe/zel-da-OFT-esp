...
bool ResourceManager::OtrSignatureCheck(const char* fileName) {
    ...
    if (nullFd >= 0) {
        ssize_t written = write(nullFd, fileName, 7);
        close(nullFd);
        if (written != 7) {
            return false;
        }
