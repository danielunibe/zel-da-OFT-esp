...
extern "C" void OTRMessage_Init() {
    ...
    if (sSpaMessageEntryTablePtr == NULL) {
        ...
        if (sSpaMessageEntryTablePtr != NULL) {
            ...
            if (f) { fprintf(f, "[COUCH] Spanish table loaded = YES\n"); fclose(f); }
            printf("[COUCH] Spanish table loaded = YES\n");
            fflush(stdout);
            SPDLOG_INFO("Spanish message table loaded successfully from es.o2r");

            // Self-test lookup validation for Mido message 0x1042
            MessageTableEntry* curr = sSpaMessageEntryTablePtr;
            while (curr->textId != 0xFFFF) {
                if (curr->textId == 0x1042) {
                    printf("[COUCH-LANG] Message ID = 0x1042\n");
                    printf("[COUCH-LANG] Requested language = 4\n");
                    printf("[COUCH-LANG] Resolved table = SPA\n");
                    printf("[COUCH-LANG] Fallback used = NO\n");
                    FILE* fdiag = fopen("logs/couch_diagnostics.txt", "a");
                    if (fdiag) {
                        fprintf(fdiag, "[COUCH-LANG] Message ID = 0x1042\n");
                        fprintf(fdiag, "[COUCH-LANG] Requested language = 4\n");
                        fprintf(fdiag, "[COUCH-LANG] Resolved table = SPA\n");
                        fprintf(fdiag, "[COUCH-LANG] Fallback used = NO\n");
                        fclose(fdiag);
                    }
                    break;
                }
            ...
            FILE* f = fopen("logs/couch_diagnostics.txt", "a");
            if (f) { fprintf(f, "[COUCH] Spanish table loaded = NO\n"); fclose(f); }
            printf("[COUCH] Spanish table loaded = NO\n");
            fflush(stdout);
            SPDLOG_WARN("Spanish message table failed to load from es.o2r");
        }
    }
...
extern "C" bool OTRMessage_IsSpanishAvailable(void) {
    return (sSpaMessageEntryTablePtr != NULL);
}
