...
extern MessageTableEntry* sSpaMessageEntryTablePtr;
...
void GameState_Update(GameState* gameState) {
    ...
    static s32 sLoggedLangState = -1;
    if (gSaveContext.language != sLoggedLangState) {
        sLoggedLangState = gSaveContext.language;
        printf("[COUCH-LANG] SaveContext.language = %d\n", gSaveContext.language);
        printf("[COUCH-LANG] Runtime language enum = %d\n", gSaveContext.language);
        printf("[COUCH-LANG] LANGUAGE_ESP = %d\n", LANGUAGE_ESP);
        printf("[COUCH-LANG] es.o2r mounted = %s\n", OTRMessage_IsSpanishAvailable() ? "YES" : "NO");
        printf("[COUCH-LANG] Spanish message table available = %s\n", (sSpaMessageEntryTablePtr != NULL) ? "YES" : "NO");
        FILE* flang = fopen("logs/couch_diagnostics.txt", "a");
        if (flang) {
            fprintf(flang, "[COUCH-LANG] SaveContext.language = %d\n", gSaveContext.language);
            fprintf(flang, "[COUCH-LANG] Runtime language enum = %d\n", gSaveContext.language);
            fprintf(flang, "[COUCH-LANG] LANGUAGE_ESP = %d\n", LANGUAGE_ESP);
            fprintf(flang, "[COUCH-LANG] es.o2r mounted = %s\n", OTRMessage_IsSpanishAvailable() ? "YES" : "NO");
            fprintf(flang, "[COUCH-LANG] Spanish message table available = %s\n", (sSpaMessageEntryTablePtr != NULL) ? "YES" : "NO");
            fclose(flang);
        }
    }
