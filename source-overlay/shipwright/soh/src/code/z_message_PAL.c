...
MessageTableEntry* sSpaMessageEntryTablePtr = NULL;
...
void Message_FindMessage(PlayState* play, u16 textId) {
    ...
    Font* font;
    const char* seg;
    const char* resolvedTableName = "ENG";
    bool fallbackUsed = false;

    if (gSaveContext.language == LANGUAGE_GER && sGerMessageEntryTablePtr != NULL) {
        messageTableEntry = sGerMessageEntryTablePtr;
        resolvedTableName = "GER";
    } else if (gSaveContext.language == LANGUAGE_FRA && sFraMessageEntryTablePtr != NULL) {
        messageTableEntry = sFraMessageEntryTablePtr;
        resolvedTableName = "FRA";
    } else if (gSaveContext.language == LANGUAGE_ESP && sSpaMessageEntryTablePtr != NULL) {
        messageTableEntry = sSpaMessageEntryTablePtr;
        resolvedTableName = "SPA";
    }

    // If selected language table is not available, default to English
    if (messageTableEntry == NULL) {
        messageTableEntry = sNesMessageEntryTablePtr;
        resolvedTableName = "ENG";
        fallbackUsed = true;
    }
    ...
    while (messageTableEntry->textId != 0xFFFF) {
        ...
        if (messageTableEntry->textId == textId) {
            ...
            static u16 sLastDiagTextId = 0;
            static u8 sLastDiagLang = 0xFF;
            if (textId != sLastDiagTextId || gSaveContext.language != sLastDiagLang) {
                sLastDiagTextId = textId;
                sLastDiagLang = gSaveContext.language;
                printf("[COUCH-LANG] Message ID = 0x%04X\n", textId);
                printf("[COUCH-LANG] Requested language = %d\n", gSaveContext.language);
                printf("[COUCH-LANG] Resolved table = %s\n", resolvedTableName);
                printf("[COUCH-LANG] Fallback used = %s\n", fallbackUsed ? "YES" : "NO");
                FILE* fl = fopen("logs/couch_diagnostics.txt", "a");
                if (fl) {
                    fprintf(fl, "[COUCH-LANG] Message ID = 0x%04X\n", textId);
                    fprintf(fl, "[COUCH-LANG] Requested language = %d\n", gSaveContext.language);
                    fprintf(fl, "[COUCH-LANG] Resolved table = %s\n", resolvedTableName);
                    fprintf(fl, "[COUCH-LANG] Fallback used = %s\n", fallbackUsed ? "YES" : "NO");
                    fclose(fl);
                }
            }
    ...
    // Message not found in selected table: Fallback to English vanilla table
    if (messageTableEntry != sNesMessageEntryTablePtr && sNesMessageEntryTablePtr != NULL) {
        MessageTableEntry* fallbackEntry = sNesMessageEntryTablePtr;
        while (fallbackEntry->textId != 0xFFFF) {
            if (fallbackEntry->textId == textId) {
                font = &play->msgCtx.font;
                font->charTexBuf[0] = fallbackEntry->typePos;
                font->msgOffset = fallbackEntry->segment;
                font->msgLength = fallbackEntry->msgSize;

                printf("[COUCH-LANG] Message ID = 0x%04X\n", textId);
                printf("[COUCH-LANG] Requested language = %d\n", gSaveContext.language);
                printf("[COUCH-LANG] Resolved table = ENG\n");
                printf("[COUCH-LANG] Fallback used = YES\n");
                FILE* fl = fopen("logs/couch_diagnostics.txt", "a");
                if (fl) {
                    fprintf(fl, "[COUCH-LANG] Message ID = 0x%04X\n", textId);
                    fprintf(fl, "[COUCH-LANG] Requested language = %d\n", gSaveContext.language);
                    fprintf(fl, "[COUCH-LANG] Resolved table = ENG\n");
                    fprintf(fl, "[COUCH-LANG] Fallback used = YES\n");
                    fclose(fl);
                }
                return;
            }
    ...
    for (i = 0; i < msgCtx->textDrawPos; i++) {
        ...
        switch (character) {
            ...
            default:
                ...
                msgCtx->textPosX += (s32)(Ship_GetCharFontWidth(character) * (R_TEXT_CHAR_SCALE / 100.0f));
                break;
        }
    }
    if (msgCtx->textDelay == 0) {
